...
async function ensureCacheTable(sql, table) {
  ...
  console.log('[neynar-cache] ensuring table', { table });
  await sql.query(`
  ...
  console.log('[neynar-cache] table ensured', { table });
}
...
export async function neynarCachedGetJson({
  ...
  if (sql) {
    try {
      ...
      if (row?.response_json && typeof row.response_json === 'object') {
        console.log('[neynar-cache] cache hit', { table, namespace, url });
        return { ok: true, status: 200, json: row.response_json, cached: true };
      }
      console.log('[neynar-cache] cache miss', { table, namespace, url });
    } catch (e) {
      console.log('[neynar-cache] cache read error', { table, namespace, url, error: e?.message || 'unknown' });
  ...
  if (sql) {
    try {
      ...
      console.log('[neynar-cache] cache upserted', { table, namespace, url, ttlSeconds: safeTtl });
    } catch (e) {
      console.log('[neynar-cache] cache write error', { table, namespace, url, error: e?.message || 'unknown' });
