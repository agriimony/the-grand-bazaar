...
export function getSnapContent(context, options = {}) {
  ...
  return {
    ...
    description: 'Inspect the current cast for a GBZ1 order, or open the bazaar directly.',
    contextLine: 'Tap Send cast context to submit a signed POST with the host cast attached when available.',
    primaryButton: {
      label: 'Send cast context',
      onPress: {
        action: 'submit',
        params: { target: submitTarget },
      },
    },
    secondaryButton: {
      label: 'Open Bazaar',
