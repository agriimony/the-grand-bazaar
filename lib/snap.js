...
function buildExpiryElements(expiry, prefix) {
  ...
  return {
    [`${prefix}ExpiryLabel`]: textElement('Expiry', { size: 'sm' }),
    [`${prefix}ExpiryValue`]: isExpired
      ? {
          type: 'badge',
          props: { label, color: 'red' },
        }
      : textElement(label, { weight: 'bold' }),
  };
}
...
export async function getSnapContent(context, options = {}) {
  ...
  if (context?.order) {
    ...
    const makerExpiryElements = buildExpiryElements(context.order?.expiry, 'maker');
    const takerExpiryElements = buildExpiryElements(context.order?.expiry, 'taker');

    return {
      title: 'Grand Bazaar Order',
      description: 'Decoded GBZ1 swap preview',
      contextLine: castHash
        ? `${makerLabel} · decoded from cast ${shortenHex(castHash, 10, 6)}`
        : `${makerLabel} · decoded from attached cast context`,
      bodyElementIds: ['comparisonRow'],
      extraElements: {
        comparisonRow: {
          type: 'stack',
          props: { direction: 'horizontal', gap: 'sm' },
          children: ['makerCard', 'swapIndicator', 'takerCard'],
        },
        makerCard: {
          type: 'stack',
          props: { gap: 'xs' },
          children: ['makerLabel', 'makerAction', 'makerAsset', 'makerExpiryLabel', 'makerExpiryValue'],
        },
        makerLabel: textElement(makerLabel, { weight: 'bold' }),
        makerAction: textElement('Giving', { size: 'sm' }),
        makerAsset: textElement(signerAsset),
        swapIndicator: {
          type: 'stack',
          props: { gap: 'xs' },
          children: ['swapArrow', 'swapWord'],
        },
        swapArrow: textElement('⇄', { weight: 'bold' }),
        swapWord: textElement('Swap'),
        takerCard: {
          type: 'stack',
          props: { gap: 'xs' },
          children: ['takerLabel', 'takerAction', 'takerAsset', 'takerExpiryLabel', 'takerExpiryValue'],
        },
        takerLabel: textElement(takerLabel, { weight: 'bold' }),
        takerAction: textElement('Receiving', { size: 'sm' }),
        takerAsset: textElement(senderAsset),
        ...makerExpiryElements,
        ...takerExpiryElements,
      },
      primaryButton: {
