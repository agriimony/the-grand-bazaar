...
async function resolveErc721Metadata(token) {
  const normalized = String(token || '').trim().toLowerCase();
  if (!ethers.isAddress(normalized)) {
    return { symbol: shortenHex(token, 8, 4), source: 'invalid' };
  }

  const cached = nftMetadataCache.get(normalized);
  if (isFresh(cached, TOKEN_METADATA_TTL_MS)) return cached.value;

  const fallback = { symbol: shortenHex(normalized, 8, 4), source: 'fallback' };
  for (const rpc of BASE_RPCS) {
    try {
      const provider = new ethers.JsonRpcProvider(rpc, undefined, { batchMaxCount: 1 });
      const symbolRaw = await readContractString(provider, normalized, [
        'function symbol() view returns (string)',
        'function symbol() view returns (bytes32)',
      ]);
      if (symbolRaw) {
        const value = { symbol: symbolRaw, source: 'rpc' };
        nftMetadataCache.set(normalized, { value, updatedAt: Date.now() });
        return value;
      }
    } catch {
      continue;
  ...
  nftMetadataCache.set(normalized, { value: fallback, updatedAt: Date.now() });
  return fallback;
}

function ipfsGatewayCandidates(uri = '') {
  const text = String(uri || '').trim();
  if (!text) return [];
  if (!text.startsWith('ipfs://')) return [text];
  const cidPath = text.replace('ipfs://', '').replace(/^ipfs\//, '');
  return [
...
async function readJsonWithTimeout(url, timeoutMs = 4000) {
...
async function readNftMetadataFromTokenUri(tokenUri = '') {
  ...
  for (const candidate of ipfsGatewayCandidates(uri)) {
    const parsed = await readJsonWithTimeout(candidate);
  ...
  const proxyUrl = `${BAZAAR_URL.replace(/\/$/, '')}/api/nft-meta?uri=${encodeURIComponent(uri)}`;
  const proxied = await readJsonWithTimeout(proxyUrl);
  if (proxied?.ok) {
    return {
      symbol: String(proxied?.symbol || '').trim(),
      name: String(proxied?.name || '').trim(),
    };
  }
...
async function resolveErc1155Metadata(token, tokenId) {
  const normalized = String(token || '').trim().toLowerCase();
  const normalizedTokenId = String(tokenId ?? '0').trim() || '0';
  if (!ethers.isAddress(normalized)) {
    return { symbol: shortenHex(token, 8, 4), source: 'invalid' };
  }

  const cacheKey = `${normalized}:${normalizedTokenId}`;
  ...
  for (const rpc of BASE_RPCS) {
    try {
      const provider = new ethers.JsonRpcProvider(rpc, undefined, { batchMaxCount: 1 });
      const contractSymbol = await readContractString(provider, normalized, [
        'function symbol() view returns (string)',
        'function symbol() view returns (bytes32)',
      ]);
      ...
      let metadataName = '';
      try {
        const contract = new ethers.Contract(normalized, ['function uri(uint256 id) view returns (string)'], provider);
        const rawUri = String(await contract.uri(BigInt(normalizedTokenId)).catch(() => '')).trim();
        if (rawUri) {
          const hexId = BigInt(normalizedTokenId).toString(16).padStart(64, '0');
          const expandedUri = rawUri.replaceAll('{id}', hexId).replace('{id}', normalizedTokenId);
          const metadata = await readNftMetadataFromTokenUri(expandedUri);
          metadataSymbol = String(metadata?.symbol || '').trim();
          metadataName = String(metadata?.name || '').trim();
        }
      } catch {
        // ignore metadata lookup failure and fall through to contract symbol / fallback
      }

      const resolvedSymbol = contractSymbol || metadataSymbol || metadataName || fallback.symbol;
      const value = {
        symbol: resolvedSymbol,
        source: contractSymbol ? 'rpc' : (metadataSymbol || metadataName ? 'metadata' : 'fallback'),
      };
      nftMetadataCache.set(cacheKey, { value, updatedAt: Date.now() });
      return value;
    } catch {
      continue;
    }
  ...
  nftMetadataCache.set(cacheKey, { value: fallback, updatedAt: Date.now() });
  return fallback;
}
...
function formatProfileLabel(profile, fallbackAddress) {
  if (profile?.username) {
    const name = `@${profile.username}`;
    if (profile?.fname && profile.fname.toLowerCase() !== profile.username.toLowerCase()) {
      return `${name} · ${profile.fname}`;
    }
    return name;
  }
  ...
  return shortenHex(fallbackAddress, 8, 4);
}
...
async function formatAsset(order, side) {
  ...
  if (label === 'ERC721') {
    const metadata = await resolveErc721Metadata(token);
    return `${metadata?.symbol || shortenHex(token, 8, 4)} #${tokenId || '0'}`;
  }

  if (label === 'ERC1155') {
    const metadata = await resolveErc1155Metadata(token, tokenId);
    return `${String(amount || '0')} ${metadata?.symbol || shortenHex(token, 8, 4)} #${tokenId || '0'}`;
  }
...
export async function getSnapContent(context, options = {}) {
  ...
  if (context?.order) {
    ...
    const makerLabel = formatProfileLabel(makerProfile, context.order?.signerWallet);
    const takerLabel = takerIsAnyone
      ? 'Anyone'
      : formatProfileLabel(takerProfile, takerWallet);
    ...
    return {
      ...
      extraElements: {
        ...
        swapIndicator: {
          ...
          children: ['swapSeparatorTop', 'swapArrow', 'swapSeparatorBottom'],
        },
        swapSeparatorTop: {
          type: 'separator',
          props: {},
