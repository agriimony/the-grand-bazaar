...
function intervalsOverlap(aStart, aEnd, bStart, bEnd) {
  return Number(aStart || 0) < Number(bEnd || 0) && Number(bStart || 0) < Number(aEnd || 0);
}
...
function buildScheduleSegments(threads, npcRegistry, loopDurationMs) {
  ...
  const participantsOverlap = (a = [], b = []) => {
    const set = new Set(a.map((v) => Number(v)).filter(Boolean));
    return b.some((v) => set.has(Number(v)));
  };

  const hasDoubleBookedParticipant = (candidate) => segments.some((seg) => (
    intervalsOverlap(candidate.startMs, candidate.endMs, seg.startMs, seg.endMs) &&
    participantsOverlap(candidate.participantIds, seg.participantIds)
  ));

  const scheduledThreadIds = new Set();
  while (cursor < loopDurationMs) {
    const waveSegments = [];
    const waveParticipants = new Set();
    let waveEndMs = cursor;
    let addedInWave = false;

    for (const thread of threads) {
      if (scheduledThreadIds.has(thread.id)) continue;
      const participantIds = [...(thread.participants || [])].map((v) => Number(v)).filter(Boolean);
      if (!participantIds.length || participantIds.some((fid) => waveParticipants.has(fid))) continue;
      ...
      if (endMs + GAP_BETWEEN_SEGMENTS_MS > loopDurationMs) continue;
      ...
      if (hasDoubleBookedParticipant(segment)) continue;

      waveSegments.push(segment);
      scheduledThreadIds.add(thread.id);
      for (const fid of participantIds) waveParticipants.add(fid);
      for (const slot of slots) occupied.add(cellKey(slot.x, slot.y));
      waveEndMs = Math.max(waveEndMs, endMs);
      idx += 1;
      addedInWave = true;
    }

    if (!addedInWave) break;
    ...
    cursor = waveEndMs + GAP_BETWEEN_SEGMENTS_MS;
  }

  segments.sort((a, b) => {
    if (Number(a.startMs || 0) !== Number(b.startMs || 0)) return Number(a.startMs || 0) - Number(b.startMs || 0);
    if (Number(a.endMs || 0) !== Number(b.endMs || 0)) return Number(a.endMs || 0) - Number(b.endMs || 0);
  ...
  while (segments.length && cursor + maxReturnMs > loopDurationMs) {
    ...
    if (!removed) break;
    cursor = segments.reduce((max, seg) => Math.max(max, Number(seg.endMs || 0) + GAP_BETWEEN_SEGMENTS_MS), 0);
    maxReturnMs = 0;
  ...
  const lastSegmentEndMs = segments.reduce((max, seg) => Math.max(max, Number(seg?.endMs || 0)), 0);
  const effectiveLoopDurationMs = Math.max(lastSegmentEndMs, segments.length ? STEP_MS : 1);

  return {
    segments,
    loopDurationMs: Math.min(loopDurationMs, effectiveLoopDurationMs),
  };
}

function buildScheduleDebugLog({ segments, threads, npcRegistry, loopStartsAt, loopDurationMs, nowMs }) {
  const threadMap = threads && typeof threads === 'object' ? threads : {};
  const npcMap = npcRegistry && typeof npcRegistry === 'object' ? npcRegistry : {};
  const loopOffset = loopDurationMs > 0
    ? ((((nowMs - loopStartsAt) % loopDurationMs) + loopDurationMs) % loopDurationMs)
    : 0;

  const activeSegments = (segments || []).filter((seg) => loopOffset >= Number(seg?.startMs || 0) && loopOffset < Number(seg?.endMs || 0));
  ...
  return {
    ...
    loopDurationMs,
    loopOffset,
    activeSegments: activeSegments.map((seg) => ({
      id: String(seg?.id || ''),
      threadId: String(seg?.threadId || ''),
      participantIds: Array.isArray(seg?.participantIds) ? seg.participantIds.map((fid) => Number(fid)).filter(Boolean) : [],
      startMs: Number(seg?.startMs || 0),
      endMs: Number(seg?.endMs || 0),
    })),
    activeSegmentId: activeSegments[0]?.id || '',
    activeSegmentIds: activeSegments.map((seg) => String(seg?.id || '')).filter(Boolean),
    segments: (segments || []).map((seg) => {
      const thread = threadMap[String(seg?.threadId || '')] || null;
      const participantLabels = (seg?.participantIds || []).map((fid) => {
        const npc = npcMap[String(fid)] || {};
        return {
          fid: Number(fid),
          name: String(npc.username || npc.displayName || fid),
          slot: (seg?.slots || []).find((slot) => Number(slot?.npcId) === Number(fid)) || null,
        };
      });
      return {
        id: String(seg?.id || ''),
        ...
        startMs: Number(seg?.startMs || 0),
        endMs: Number(seg?.endMs || 0),
        durationMs: Math.max(0, Number(seg?.endMs || 0) - Number(seg?.startMs || 0)),
        activeNow: loopOffset >= Number(seg?.startMs || 0) && loopOffset < Number(seg?.endMs || 0),
        participants: participantLabels,
        events: Array.isArray(thread?.events)
          ? thread.events.map((evt) => ({
              id: String(evt?.id || ''),
              speakerId: Number(evt?.speakerId || 0),
              speaker: String(npcMap[String(evt?.speakerId)]?.username || npcMap[String(evt?.speakerId)]?.displayName || evt?.speakerId || ''),
              kind: String(evt?.kind || ''),
              durationMs: Number(evt?.durationMs || 0),
              castHash: String(evt?.castHash || ''),
...
export async function getWorldNpcs({ world, channelId, debug = false }) {
  ...
  const schedulePlan = buildScheduleSegments(threadsList, fullNpcRegistry, LOOP_DURATION_MS);
  const segments = Array.isArray(schedulePlan?.segments) ? schedulePlan.segments : [];
  const effectiveLoopDurationMs = Number(schedulePlan?.loopDurationMs || LOOP_DURATION_MS) || LOOP_DURATION_MS;
  ...
  return {
    ...
    body: {
      ...
      loopDurationMs: effectiveLoopDurationMs,
      npcs: npcRegistry,
      threads,
      segments,
      motion: {
